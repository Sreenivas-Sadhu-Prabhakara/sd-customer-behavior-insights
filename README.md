...
| **Asset Type** | Customer Behavior Model |
| **Control Record** | Customer Behavior Model Analysis |
...
| POST | `/v1/customer-behavior-model-analysis/initiate` | Initiate |
| GET | `/v1/customer-behavior-model-analysis` | Retrieve (list) |
| GET | `/v1/customer-behavior-model-analysis/{crId}/retrieve` | Retrieve |
| PUT | `/v1/customer-behavior-model-analysis/{crId}/update` | Update |
| PUT | `/v1/customer-behavior-model-analysis/{crId}/control` | Control — body `{"action": "suspend"\|"resume"\|"terminate"}` |
...
curl -X POST localhost:8080/v1/customer-behavior-model-analysis/initiate -H 'content-type: application/json' -d '{"note":"hello"}'
```
...
```bash
mvn -B verify
docker build -t bian/sd-customer-behavior-insights:0.1.0 .
```

## Deploy (Helm → K8s with Cilium mesh)

```bash
helm upgrade --install sd-customer-behavior-insights ./helm -n bian-sales-service
```

Exposed through the platform Gateway at path prefix `/sd-customer-behavior-insights` (Cilium Gateway API). Mesh policy (`CiliumNetworkPolicy`) allows: gateway ingress, same-area peers, Prometheus — everything else denied.
